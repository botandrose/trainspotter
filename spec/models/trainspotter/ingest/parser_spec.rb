...
RSpec.describe Trainspotter::Ingest::Parser do
  describe "#parse_line" do
    ...
    context "with params line" do
      it "creates a params entry with parsed params" do
        parser.parse_line('Started GET "/" for 127.0.0.1 at 2024-01-06 10:00:00 +0000')
        entry = parser.parse_line('  Parameters: {"email"=>"test@example.com"}')

        expect(entry.type).to eq(:params)
        expect(entry.metadata[:params]).to eq({ "email" => "test@example.com" })
      end
    end
    ...
    context "with unrecognized line" do
      it "creates an other entry" do
        parser.parse_line('Started GET "/" for 127.0.0.1 at 2024-01-06 10:00:00 +0000')
        entry = parser.parse_line("  Some unrecognized log line")

        expect(entry.type).to eq(:other)
        expect(entry.raw).to eq("  Some unrecognized log line")
      end
    end
